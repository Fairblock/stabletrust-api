...
app.post("/transfer", async (req, res) => {
  const { privateKey, recipientAddress, tokenAddress, amount, useOffchainVerify, waitForFinalization } = req.body;
...
app.post("/withdraw", async (req, res) => {
  const { privateKey, tokenAddress, amount, useOffchainVerify, waitForFinalization } = req.body;
...
app.post("/balance", async (req, res) => {
  ...
  try {
    ...
    const balance = await sharedClient.getConfidentialBalance(queryAddress, elgamalKey, tokenAddress);
...
app.listen(PORT, () => {
  ...
  console.log(`  POST /deposit   — deposit ERC-20 tokens into confidential account`);
  console.log(`  POST /transfer  — confidential token transfer between accounts`);
  console.log(`  POST /withdraw  — withdraw confidential tokens to public ERC-20`);
  console.log(`  POST /balance   — get decrypted confidential balance for a wallet`);
});
